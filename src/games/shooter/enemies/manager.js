...
import { rand, randInt, clamp, damp, wrapAngle, angleLerp, alignYAxis, TAU } from '../util.js';
...
export class EnemyManager {
  ...
  nearestTarget(pos) {
    ...
    for (const t of this.targets()) {
      if (!t.alive) continue;
      const d = t.body.pos.distanceToSquared(pos);
      if (d < bd) {
        bd = d;
        best = t;
      }
  ...
  spawn(type, pos, id = null) {
    ...
    const solid = makeInkMaterial({
      ink: T.ink === INK.BLACK ? INK.RED : INK.BLACK,
      fill: true,
      side: THREE.DoubleSide,
    });
    const model =
      T.model === 'bomber'
        ? buildBomber(mat, solid, T)
        : T.model === 'blob'
          ? buildBomber(mat, solid, T, true)
          : T.model === 'flyer'
            ? buildFlyer(mat, solid, T)
            : buildHumanoid(mat, solid, T);
    ...
    const e = {
      ...
      state: 'spawn',
      t: 0,
      body: makeBody(pos, hw, (T.flying ? 0.8 : 1.85) * T.scale, T.boss ? 1.2 : 0.6),
      ...
      appR: 0,
      appT: rand(0, 2),
      keepMul: rand(0.75, 1.35),
      backoffT: 0,
      hitSpheres: model.hit.map(() => new THREE.Vector3()),
      fuseT: -1,
      shieldHp: T.shield ? 2 : 0,
      flyState: 'orbit',
      flyT: rand(0, 3),
    ...
    this.enemies.push(e);
    this.alive++;
    this.ctx.effects.strokeBurst(
      pos.clone().add(_v.set(0, 1, 0)),
      T.ink ?? INK.RED,
      T.boss ? 60 : 26,
      T.boss ? 10 : 6,
      { life: 0.5, size: 0.03 },
  ...
  nearestVisible(from, forward, cosHalf, maxDist) {
    ...
    for (const e of this.enemies) {
      if (!e.alive) continue;
      _v.subVectors(e.center, from);
      const d = _v.length();
      if (d > bestD || d < 0.01) continue;
      if (_v.divideScalar(d).dot(forward) < cosHalf) continue;
      if (!this.ctx.world.hasLineOfSight(from, e.center, SEE_THROUGH)) continue;
      best = e;
      bestD = d;
    }
    return best;
  ...
  raycast(o, d, maxDist, ignore = null) {
    ...
    for (const e of this.enemies) {
      ...
      for (let i = 0; i < e.hit.length; i++) {
        const c = e.hitSpheres[i],
          r = e.hit[i][1] * e.T.scale;
        _v.subVectors(c, o);
        const tca = _v.dot(d);
  ...
  inArc(pos, dir, range, cosHalf) {
    ...
    for (const e of this.enemies) {
      if (!e.alive) continue;
      _v.subVectors(e.center, pos);
      const d = _v.length() - (e.T.boss ? 1.2 : 0);
      if (d > range + 0.3) continue;
      if (d > 0.3 && _v.normalize().dot(dir) < cosHalf) continue;
      out.push({ enemy: e, dist: d });
    }
  ...
  blastEnemies(point, radius, dmg, except) {
    for (const e of this.enemies) {
      ...
      _d.subVectors(e.center, point).normalize();
      this.damage(e, dmg * (1 - (d / radius) * 0.6), {
        point: e.center.clone(),
        dir: _d.clone(),
        part: 'torso',
        source: 'blast',
        crit: false,
  ...
  yank(e, target, authoritative = false) {
    ...
    e.path = null;
    e.flyState = 'stunned';
    _d.subVectors(target, e.body.pos);
    const dist = _d.length();
    _d.divideScalar(Math.max(dist, 0.01));
    e.body.vel.copy(_d).multiplyScalar(clamp(dist * 1.6, 10, 26));
    e.body.vel.y = clamp(dist * 0.5, 4, 9);
  ...
  _breakShield(e) {
    ...
    this.ctx.scene.attach(g);
    this.ctx.effects.debris(
      g,
      g.position,
      new THREE.Vector3(rand(-3, 3), 4, rand(-3, 3)),
      new THREE.Vector3(rand(-6, 6), rand(-6, 6), rand(-6, 6)),
      { radius: 0.4, blood: false, life: 8 },
    );
    e.hit = e.hit.filter((h) => h[0] !== 'shield');
    e.hitSpheres = e.hit.map(() => new THREE.Vector3());
    e.J.shieldG = null;
    audio.shieldHit(e.center);
    this.ctx.game.addScore(40, '破盾');
  }
  kill(e, info) {
    if (!e.alive) return;
    e.alive = false;
    ...
    if (e.T.weapon === 'bomb') {
      this._explodeBomber(e, 0.8);
      if (this.onKill) this.onKill(e, info, true);
      return;
    }
    ...
    if (e.T.flying) {
      ...
      eff.debris(
        ...
        dir
          .clone()
          .multiplyScalar(4)
          .add(_v.set(rand(-2, 2), 1, rand(-2, 2))),
        new THREE.Vector3(rand(-9, 9), rand(-9, 9), rand(-9, 9)),
        { radius: 0.5, blood: true, life: 8 },
      );
      eff.blood(e.center, dir, 1);
      if (this.onKill) this.onKill(e, info, true);
      return;
    }
    const over =
      ...
      info.source === 'deflect' ||
      info.source === 'blast';
    const detach = (obj, extraVel, radius) => {
      if (!obj || !obj.parent) return;
      obj.updateWorldMatrix(true, false);
      scene.attach(obj);
      _v.copy(dir).multiplyScalar(rand(3, 7)).add(extraVel);
      _v.y += rand(2, 5);
      eff.debris(obj, obj.position, _v, new THREE.Vector3(rand(-8, 8), rand(-8, 8), rand(-8, 8)), {
        radius,
        blood: true,
        life: rand(7, 10),
      });
    ...
    if (over) {
      audio.gib(e.center);
      const J = e.J;
      if (info.crit || ((info.source === 'katana' || info.source === 'focus') && Math.random() < 0.35)) {
        detach(J.headG, _v2.set(rand(-2, 2), 3, rand(-2, 2)), 0.25);
        eff.fountain(
          e.parts.torso.getWorldPosition(new THREE.Vector3()).add(_v2.set(0, 0.35, 0)),
          _up,
          0.9,
          inkC,
        );
      ...
      if (info.source === 'katana' || info.source === 'focus') {
        const r = Math.random();
        if (r < 0.4) detach(J.armR, _v2.set(rand(-3, 3), 2, rand(-3, 3)), 0.12);
        else if (r < 0.7) detach(J.armL, _v2.set(rand(-3, 3), 2, rand(-3, 3)), 0.12);
        else {
          detach(J.torso, _v2.set(rand(-2, 2), 2, rand(-2, 2)), 0.3);
          eff.fountain(e.parts.hips.getWorldPosition(new THREE.Vector3()), _up, 0.7, inkC);
        }
      } else if (info.source === 'deflect' || info.source === 'blast' || -e.hp > e.maxHp * 0.6) {
        const limbs = [J.armL, J.armR, J.legL, J.legR, J.torso];
        const k = e.T.boss ? 5 : randInt(1, 2);
        for (let i = 0; i < k && limbs.length; i++) {
          const o = limbs.splice(randInt(0, limbs.length - 1), 1)[0];
          detach(o, _v2.set(rand(-3, 3), 2, rand(-3, 3)), 0.15);
        }
      }
    }
    e.topple = {
      axis: Math.random() < 0.5 ? 'x' : 'z',
      sign: dir.z > 0 || Math.random() < 0.5 ? 1 : -1,
      t: 0,
    };
    eff.bloodPool(e.body.pos, rand(1.1, 1.8) * (e.T.boss ? 2.5 : 1), inkC);
    eff.blood(e.center, dir, 1.2, { ink: inkC });
    if (Math.random() < 0.6 && e.J.gun && e.J.gun.parent)
      detach(e.J.gun, _v2.set(rand(-2, 2), 2, rand(-2, 2)), 0.08);
    if (e.J.shieldG) this._breakShield(e);
    if (e.T.boss) {
      eff.explosion(e.center, 6, INK.BLACK);
      audio.explosion(e.center);
      if (this.onBoss) this.onBoss(e);
    }
  ...
  _explodeBomber(e, scale = 1) {
    ...
    this.ctx.effects.explosion(c, T.blast * scale, INK.BLACK);
    audio.explosion(c);
    for (const P of this.targets()) {
      const d = P.center.distanceTo(c);
      if (P.alive && d < T.blast * scale) {
        P.takeDamage(T.dmg * this.mods.damage * Math.sqrt(1 - d / (T.blast * scale)), c);
        P.knockback(_v.subVectors(P.center, c).normalize(), 7);
      }
    }
    ...
    if (e.alive) {
      ...
      e.state = 'dead';
      this.alive--;
      if (this.onKill) this.onKill(e, { source: 'blast', dir: _up.clone() }, true);
    }
    disposeTree(e.root);
    e.rootDetached = true;
    e.deadT = 99;
  ...
  update(dt) {
    ...
    for (const e of this.enemies) {
      e.t += dt;
      if (!e.alive) {
        this._updateDead(e, dt);
        continue;
      }
      const P = this._pickTarget(e, dt) || ctx.player;
      const pp = P.body.pos,
        pc = P.center;
      if (e.flashT > 0) {
        e.flashT -= dt;
      ...
      if (e.T.flying) {
        this._thinkFlyer(e, dt, pc, P);
        world.moveBody(e.body, dt);
      } else {
        if (e.state === 'stunned') {
          if (e.t > e.stunDur) e.state = 'hunt';
        } else if (P.alive) this._think(e, dt, pp, pc, P);
        else this._wander(e, dt);
        e.body.vel.y -= 24 * dt;
        world.moveBody(e.body, dt);
      }
      if (e.body.pos.y < -6) {
        this.kill(e, { source: 'fall', dir: _up.clone() });
        continue;
      }
      ...
      if (!e.T.flying) {
        ...
        const ox = e.body.pos.x - pp.x,
          oz = e.body.pos.z - pp.z;
        const o2 = ox * ox + oz * oz;
        if (o2 < pr * pr && Math.abs(e.body.pos.y - pp.y) < 1.7) {
          const od = Math.sqrt(o2) || 0.0001;
          const push = pr - od;
          e.body.pos.x += (ox / od) * push;
          e.body.pos.z += (oz / od) * push;
          if (world.overlapsBody(e.body)) {
            e.body.pos.x -= (ox / od) * push;
            e.body.pos.z -= (oz / od) * push;
          }
        }
      }
      if (e.T.flying) this._animateFlyer(e, dt);
      else this._animate(e, dt, pc);
      this._syncHit(e);
    }
    this._separate(dt);
  ...
  _syncHit(e) {
    ...
    for (let i = 0; i < e.hit.length; i++)
      e.hitSpheres[i].setFromMatrixPosition(e.parts[e.hit[i][0]].matrixWorld);
    e.center.setFromMatrixPosition(e.parts.torso.matrixWorld);
  }
  _updateDead(e, dt) {
    e.deadT += dt;
    const tp = e.topple;
    if (!tp || e.rootDetached) return;
    if (tp.t < 1) {
      tp.t = Math.min(1, tp.t + dt * 2.2);
      const a = tp.sign * (Math.PI / 2) * (1 - Math.pow(1 - tp.t, 2));
      if (tp.axis === 'x') e.J.hips.parent.rotation.x = a;
      else e.J.hips.parent.rotation.z = a;
    }
    if (e.deadT > 8.3) e.root.scale.setScalar(Math.max(0.001, ((9 - e.deadT) / 0.7) * e.T.scale));
    e.root.rotation.y = e.yaw;
  }
  // ponytail: O(n²) separation for the small active wave; use spatial buckets if profiling shows a bottleneck.
  _separate(dt) {
    this._sepT -= dt;
    if (this._sepT > 0) return;
    this._sepT = 0.05;
    ...
    for (let i = 0; i < es.length; i++) {
      ...
      for (let j = i + 1; j < es.length; j++) {
        const b = es[j];
        if (!b.alive || b.T.flying) continue;
        const dx = b.body.pos.x - a.body.pos.x,
          dz = b.body.pos.z - a.body.pos.z;
        const d2 = dx * dx + dz * dz;
        const rr = a.body.halfW + b.body.halfW + 0.75;
        if (d2 > rr * rr || d2 < 1e-6 || Math.abs(b.body.pos.y - a.body.pos.y) > 1.5) continue;
        const d = Math.sqrt(d2);
        const push = (rr - d) * 9;
        a.body.vel.x -= (dx / d) * push;
        a.body.vel.z -= (dz / d) * push;
        b.body.vel.x += (dx / d) * push;
        b.body.vel.z += (dz / d) * push;
      }
    }
  }
  _steer(e, dt, gx, gz, speed, accel) {
    ...
    dx /= l;
    dz /= l;
    const a = (b.onGround ? accel : accel * 0.3) * dt;
    speed *= this.mods.speed;
    b.vel.x += clamp(dx * speed - b.vel.x, -a, a);
    b.vel.z += clamp(dz * speed - b.vel.z, -a, a);
  ...
  _approachPoint(e, dt, target, out) {
    ...
    if (e.appT <= 0) {
      e.appT = rand(2.5, 5);
      e.appAng += rand(-0.7, 0.7); // drift, keep the slot
  ...
  _follow(e, dt, target, speed) {
    ...
    const stale =
      !e.path ||
      e.pathI >= e.path.length ||
      (e.pathT <= 0 && (!e.pathGoal || e.pathGoal.distanceTo(target) > 3.5 || !e.path.complete));
    if (stale && (e.pathT <= 0 || !e.path)) {
      e.pathT = 0.8 + rand(0, 0.6);
      const p = nav.findPath(b.pos, target);
      if (p && p.length) {
        e.path = p;
        e.pathI = 0;
        ...
        while (
          e.pathI < p.length - 1 &&
          Math.hypot(p[e.pathI].x - b.pos.x, p[e.pathI].z - b.pos.z) < 0.7 &&
          Math.abs(p[e.pathI].y - b.pos.y) < 1
        )
          e.pathI++;
      }
    }
    ...
    if (e.path && e.pathI < e.path.length) {
      const nd = e.path[e.pathI];
      const hd = Math.hypot(nd.x - b.pos.x, nd.z - b.pos.z);
      if (hd < 0.5 && Math.abs(nd.y - b.pos.y) < 1.2) {
        e.pathI++;
        if (e.pathI < e.path.length) goal = e.path[e.pathI];
      } else goal = nd;
    }
    if (!goal) goal = target;
    ...
    if (b.onGround) {
      if (goal.y > b.pos.y + 0.6 && hd < 1.7) {
        b.vel.y = 9;
        b.onGround = false;
      } else if (b.hitWall) {
        e.stuckT += dt;
        if (e.stuckT > 0.35 && e.stuckT < 0.4) e.pathT = 0;
        if (e.stuckT > 0.9) {
          b.vel.y = 9;
          b.onGround = false;
          e.stuckT = 0;
  ...
  _wander(e, dt) {
    e.body.vel.x = damp(e.body.vel.x, 0, 6, dt);
    e.body.vel.z = damp(e.body.vel.z, 0, 6, dt);
  ...
  _think(e, dt, pp, pc, P) {
    ...
    if (T.weapon === 'bomb') {
      if (e.fuseT >= 0) {
        e.fuseT -= dt;
        b.vel.x = damp(b.vel.x, 0, 4, dt);
        b.vel.z = damp(b.vel.z, 0, 4, dt);
        e.yawT = yawTo;
        e.flashT = 0.02;
        if (!e.flashOn) {
          setFill(e.mat, true);
        ...
        if (Math.floor(e.fuseT * 8) !== Math.floor((e.fuseT + dt) * 8)) audio.fuse(e.center);
        if (e.fuseT <= 0) this._explodeBomber(e);
        return;
      }
      if (dist < T.fuseRange && Math.abs(dy) < 2.2 && e.los) {
        e.fuseT = T.fuse;
        audio.fuse(e.center);
        return;
      }
      if (e.los && dist < 12 && Math.abs(dy) < 1.5) this._steer(e, dt, pp.x, pp.z, T.speed, 45);
        ...
        if (e.attackT < 0.18 && !e.attackHit) {
          ...
          for (let i = 0; i < 5; i++) {
            const a0 = -0.9 + i * 0.45,
              a1 = a0 + 0.45;
            _v.set(
              e.center.x + Math.sin(e.yaw + a0) * 1.5,
              e.center.y + 0.5 - i * 0.18,
              e.center.z + Math.cos(e.yaw + a0) * 1.5,
            );
            _v2.set(
              e.center.x + Math.sin(e.yaw + a1) * 1.5,
              e.center.y + 0.5 - (i + 1) * 0.18,
              e.center.z + Math.cos(e.yaw + a1) * 1.5,
            );
            ctx.effects.tracer(_v, _v2, INK.RED, 0.025, 0.16);
          }
          if (dist < T.reach && Math.abs(dy) < 1.7) {
            if (P.tryBlockMelee(e)) {
              e.state = 'stunned';
              e.t = 0;
              e.stunDur = 1.1;
              b.vel.set((-dx / dist) * 7, 3.5, (-dz / dist) * 7);
            } else P.takeDamage(T.dmg * this.mods.damage, e.center);
          } else audio.katanaSwing();
          e.cool = rand(T.cool[0], T.cool[1]);
          e.backoffT = rand(0.45, 0.75);
        }
        return;
      }
    ...
    if (T.weapon === 'boss') {
      this._thinkBoss(e, dt, pp, pc, dist, dy, yawTo, P);
      return;
    }
    const inRange = e.los && dist < T.range;
    if (inRange) {
      e.aimAmt = damp(e.aimAmt, 1, 8, dt);
      e.yawT = yawTo;
      let mx = 0,
        mz = 0;
      const nx = dx / dist,
      ...
      if (T.stationary) {
        ...
        this._follow(e, dt, pp, T.speed * 0.8);
      ...
      else if (Math.abs(dy) > 1.2) {
        this._follow(e, dt, pp, T.speed * 0.85);
        this._shoot(e, dt, pc);
        e.yawT = yawTo;
        return;
      } else if (dist < T.keep * e.keepMul) {
        mx = -nx;
        mz = -nz;
      } else if (dist > T.range * 0.7 && T.weapon === 'shotgun') {
        mx = nx;
        mz = nz;
      } else {
        e.strafeT -= dt;
        if (e.strafeT <= 0) {
          e.strafeT = rand(0.8, 2);
          e.strafeDir *= -1;
        }
        mx = -nz * e.strafeDir;
        mz = nx * e.strafeDir;
  ...
  _thinkBoss(e, dt, pp, pc, dist, dy, yawTo, P) {
    ...
    if (e.bossAtk) {
      ...
      a.t += dt;
      e.yawT = yawTo;
      b.vel.x = damp(b.vel.x, 0, 5, dt);
      b.vel.z = damp(b.vel.z, 0, 5, dt);
      if (a.kind === 'stomp') {
        if (a.t > 0.75 && !a.done) {
          a.done = true;
          audio.stomp(e.center);
          ctx.effects.shakeAmt += 0.9;
          ctx.effects.explosion(b.pos.clone().add(_v.set(0, 0.2, 0)), 7, INK.BLACK);
          for (let i = 0; i < 24; i++) {
            const an = (i / 24) * TAU;
            _v.set(b.pos.x + Math.cos(an) * 3, b.pos.y + 0.3, b.pos.z + Math.sin(an) * 3);
            _v2.set(Math.cos(an) * 14, 2, Math.sin(an) * 14);
            ctx.effects._spawn('stroke', _v, _v2, {
              size: 0.06,
              life: 0.4,
              ink: INK.BLACK,
              gravity: 4,
          ...
          for (const t of this.targets()) {
            ...
            if (t.alive && dd < 7.5 && t.body.pos.y < b.pos.y + 2.5) {
              t.takeDamage(T.dmg * this.mods.damage, e.center);
              t.knockback(_d.subVectors(t.center, e.center).normalize(), 9);
            }
          }
        ...
        if (a.t > 0.6 && !a.done) {
          a.done = true;
          _v.setFromMatrixPosition(e.parts.head.matrixWorld);
          _v.y += 1;
          _d.subVectors(pc, _v);
          const l = _d.length();
          _d.divideScalar(l);
          _d.y += l * 0.012;
          _d.normalize();
          this.projectiles.fire(_v, _d, 24, T.dmg * 0.9, e, INK.BLACK, 0.4, 1);
          audio.enemyShot(e.center);
        }
        if (a.t > 1.0) {
          e.bossAtk = null;
          e.cool = rand(T.cool[0] * 0.6, T.cool[1] * 0.6);
        }
      }
      return;
    }
    ...
    if (e.cool <= 0 && e.los) {
      if (dist < 7 && Math.abs(dy) < 3) {
        e.bossAtk = { kind: 'stomp', t: 0, done: false };
        audio.bossRoar(e.center);
        return;
      }
      if (dist < T.range) {
        e.bossAtk = { kind: 'throw', t: 0, done: false };
        return;
      }
    }
    if (e.los && dist < 14 && Math.abs(dy) < 2) this._steer(e, dt, pp.x, pp.z, T.speed, 30);
    else this._follow(e, dt, pp, T.speed);
    if (e.los) e.yawT = yawTo;
  }
  ...
  _thinkEraser(e, dt, pp, pc, dist, dy, yawTo, P) {
    ...
    if (e.bossAtk) {
      ...
      if (a.kind === 'charge') {
        if (a.t < 0.7) {
          ...
          b.vel.z = damp(b.vel.z, 0, 8, dt);
          if (a.t > 0.55 && !a.dir) {
            a.dir = new THREE.Vector3(pp.x - b.pos.x, 0, pp.z - b.pos.z).normalize();
            audio.bossRoar(e.center);
          }
        } else if (a.t < 1.9) {
          b.vel.x = a.dir.x * 17 * this.mods.speed;
          b.vel.z = a.dir.z * 17 * this.mods.speed;
          for (const t of this.targets()) {
            if (!t.alive) continue;
            const d = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z);
            if (d < 2.6 && Math.abs(t.body.pos.y - b.pos.y) < 3 && !a.hit) {
              a.hit = true;
              t.takeDamage(T.dmg * this.mods.damage, e.center);
              t.knockback(_d.subVectors(t.center, e.center).setY(0.2).normalize(), 13);
              ctx.effects.shakeAmt += 0.5;
            }
          }
          if (b.hitWall) {
            a.t = 1.9;
            ctx.effects.strokeBurst(e.center, INK.PINK, 30, 9, { life: 0.4, size: 0.05 });
            audio.stomp(e.center);
            ctx.effects.shakeAmt += 0.6;
          }
          if (Math.floor(a.t * 10) !== Math.floor((a.t - dt) * 10))
            ctx.effects.strokeBurst(b.pos.clone().add(_v.set(0, 0.3, 0)), INK.PINK, 4, 3, {
              life: 0.35,
              size: 0.06,
            });
        } else {
        ...
        if (a.t > 0.9 && !a.done) {
          ...
          for (let i = 0; i < 2 && live + i < 4; i++) {
            const an = rand(0, TAU);
            const sp = this.spawn('bomber', b.pos.clone().add(_v.set(Math.cos(an) * 3, 0, Math.sin(an) * 3)));
            sp.state = 'hunt';
            sp.root.scale.setScalar(sp.T.scale);
          }
          for (const t of this.targets()) {
            const d = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z);
            if (t.alive && d < 6.5 && t.body.pos.y < b.pos.y + 3) {
              t.takeDamage(T.dmg * 0.8 * this.mods.damage, e.center);
              t.knockback(_d.subVectors(t.center, e.center).normalize(), 9);
            }
          }
        }
    ...
    if (e.cool <= 0 && e.los) {
      if ((e.charges || 0) % 3 === 2 && dist < 12) {
        e.bossAtk = { kind: 'rub', t: 0, done: false };
        e.charges++;
        return;
      }
      if (dist < T.range && dist > 3) {
        e.bossAtk = { kind: 'charge', t: 0, hit: false, dir: null };
        return;
      }
    }
    if (e.los && dist < 16 && Math.abs(dy) < 2) this._steer(e, dt, pp.x, pp.z, T.speed, 30);
    else this._follow(e, dt, pp, T.speed);
    if (e.los) e.yawT = yawTo;
  }
  ...
  _thinkInkblot(e, dt, pp, pc, dist, dy, yawTo, P) {
    ...
    if (e.bossAtk) {
      ...
      if (a.kind === 'spray') {
        if (a.t > 0.6 && a.shots < 9 && a.t > 0.6 + a.shots * 0.09) {
          _v.setFromMatrixPosition(e.parts.torso.matrixWorld);
          _v.y += 0.8;
          const base = Math.atan2(pp.x - b.pos.x, pp.z - b.pos.z);
          const an = base + (a.shots - 4) * 0.19;
          _d.set(Math.sin(an), 0.18 + rand(-0.05, 0.05), Math.cos(an)).normalize();
          this.projectiles.fire(_v, _d, 20, T.dmg * 0.55 * this.mods.damage, e, INK.BLACK, 0.3, 1);
          a.shots++;
          if (a.shots === 1) audio.enemyShot(e.center);
        }
        if (a.t > 1.8) {
          e.bossAtk = null;
          e.cool = rand(T.cool[0], T.cool[1]);
          e.sprays = (e.sprays || 0) + 1;
        }
      } else {
        if (a.t > 0.8 && !a.done) {
          a.done = true;
          audio.bossRoar(e.center);
          ctx.effects.strokeBurst(e.center, INK.BLACK, 40, 10, { life: 0.5, size: 0.05 });
          const live = this.enemies.filter((x) => x.alive && x.type === 'flyer').length;
          for (let i = 0; i < 3 && live + i < 6; i++) {
            const sp = this.spawn('flyer', e.center.clone().add(_v.set(rand(-2, 2), 2 + i, rand(-2, 2))));
            sp.state = 'hunt';
            sp.root.scale.setScalar(sp.T.scale);
          }
        }
    ...
    e.hopT = (e.hopT ?? 1) - dt;
    if (b.onGround && e.hopT <= 0 && dist > 4) {
      e.hopT = rand(1.6, 2.4);
      const nx = (pp.x - b.pos.x) / dist,
        nz = (pp.z - b.pos.z) / dist;
      b.vel.set(nx * 11, 13, nz * 11);
      b.onGround = false;
      e.hopping = true;
    }
    if (e.hopping && b.onGround) {
      e.hopping = false;
      audio.stomp(e.center);
      ctx.effects.shakeAmt += 0.4;
      ctx.effects.bloodPool(b.pos, 3.5, INK.BLACK);
      for (const t of this.targets()) {
        const d = Math.hypot(t.body.pos.x - b.pos.x, t.body.pos.z - b.pos.z);
        if (t.alive && d < 4.5 && Math.abs(t.body.pos.y - b.pos.y) < 2.5) {
          t.takeDamage(T.dmg * 0.7 * this.mods.damage, e.center);
          t.knockback(_d.subVectors(t.center, e.center).normalize(), 8);
        }
      }
    }
    if (e.cool <= 0 && e.los) {
      if ((e.sprays || 0) % 3 === 2) {
        e.bossAtk = { kind: 'summon', t: 0, done: false };
        e.sprays++;
        return;
      }
      if (dist < T.range) {
        e.bossAtk = { kind: 'spray', t: 0, shots: 0 };
        return;
      }
    }
    if (!e.hopping) {
  ...
  _thinkFlyer(e, dt, pc, P) {
    ...
    e.cool -= dt;
    const want = _v2;
    if (e.flyState === 'stunned') {
      b.vel.y -= 20 * dt;
      if (b.onGround || e.t > 2.2) {
        e.flyState = 'climb';
        e.flyT = 1.2;
        e.state = 'hunt';
      }
    } else if (e.flyState === 'orbit') {
      const ang = Math.atan2(b.pos.x - pc.x, b.pos.z - pc.z) + e.orbitDir * 0.45;
      const rad = 11;
      want.set(pc.x + Math.sin(ang) * rad, pc.y + 6 + Math.sin(e.t * 1.3) * 1.5, pc.z + Math.cos(ang) * rad);
      this._flyTo(e, want, T.speed, 22, dt);
      if (e.cool <= 0 && P.alive && ctx.world.hasLineOfSight(e.center, pc, SEE_THROUGH)) {
        e.flyState = 'dive';
        e.flyT = 1.6;
        e.diveHit = false;
        audio.flyerDive(e.center);
      }
      if (Math.random() < dt * 1.5) audio.flyerBuzz(e.center);
    } else if (e.flyState === 'dive') {
      want.copy(pc);
      this._flyTo(e, want, 16, 28, dt);
      ...
      if (d < 1.4 && !e.diveHit) {
        ...
        if (P.tryBlockMelee(e)) {
          ...
          e.state = 'stunned';
          e.t = 0;
          b.vel.set(-b.vel.x * 0.3, -3, -b.vel.z * 0.3);
        } else if (P.alive) {
          P.takeDamage(T.dmg * this.mods.damage, e.center);
          P.knockback(_d.copy(b.vel).normalize(), 3);
        }
      }
      if (e.flyT <= 0 || e.diveHit || b.hitWall) {
        e.flyState = 'climb';
        e.flyT = 1.1;
        e.cool = rand(T.cool[0], T.cool[1]);
      }
    } else {
      want.set(pc.x + (b.pos.x - pc.x) * 1.5, pc.y + 8, pc.z + (b.pos.z - pc.z) * 1.5);
      this._flyTo(e, want, T.speed, 18, dt);
      if (e.flyT <= 0) e.flyState = 'orbit';
    }
    // avoid geometry: look ahead and above the ground
    const sp = b.vel.length();
    if (sp > 0.5) {
      _d.copy(b.vel).divideScalar(sp);
      const h = ctx.world.raycast(e.center, _d, 3.5);
      if (h) {
        b.vel.addScaledVector(h.normal, 14 * dt);
        b.vel.y += 12 * dt;
      }
    }
    const g = ctx.world.raycast(e.center, _v3.set(0, -1, 0), 2.5);
    if (g) b.vel.y += 12 * dt;
    _d.copy(b.vel);
    if (_d.lengthSq() > 0.1) e.yawT = Math.atan2(_d.x, _d.z);
  }
  _flyTo(e, target, speed, accel, dt) {
    const b = e.body;
    _d.subVectors(target, b.pos);
    const l = _d.length();
    if (l < 0.3) {
      b.vel.multiplyScalar(Math.max(0, 1 - 4 * dt));
      return;
    }
    _d.divideScalar(l).multiplyScalar(speed * this.mods.speed);
    _v3.subVectors(_d, b.vel);
    const m = _v3.length();
    if (m > accel * dt) _v3.multiplyScalar((accel * dt) / m);
    b.vel.add(_v3);
  }
  _shoot(e, dt, pc, P) {
    const T = e.T,
      ctx = this.ctx;
    const muzzle = _v.setFromMatrixPosition(e.tip.matrixWorld);
    if (T.weapon === 'sniper') {
      if (e.cool > 0) {
        this._hideLaser(e);
        return;
      ...
      if (e.aimT > T.aimTime * 0.5 && !e.aimWarned) {
        e.aimWarned = true;
        audio.sniperAim(e.center);
      }
      if (e.aimT >= T.aimTime) {
        e.aimT = 0;
        e.aimWarned = false;
        e.cool = rand(T.cool[0], T.cool[1]);
        this._fireOne(e, muzzle, e.aimPoint, T.spread, T.pspeed, T.dmg, 0.07, P);
        audio.sniperShot(e.center);
        this._hideLaser(e);
        e.aimPoint = null;
      }
      return;
    ...
    if (e.burstLeft > 0) {
      ...
      if (e.burstT <= 0) {
        ...
        audio.enemyShot(e.center);
        if (e.burstLeft === 0) e.cool = rand(T.cool[0], T.cool[1]);
      }
      return;
    }
    if (e.cool <= 0) {
      if (T.weapon === 'shotgun') {
        for (let i = 0; i < T.pellets; i++)
          this._fireOne(e, muzzle, pc, T.spread, T.pspeed * rand(0.85, 1.1), T.dmg, 0.05, P);
  ...
  _removeLaser(e) {
    if (e.laser) {
      this.ctx.scene.remove(e.laser);
      e.laser.geometry.dispose();
      e.laser.material.dispose();
      e.laser = null;
    }
  }
  _fireOne(e, muzzle, pc, spread, speed, dmg, thick, P = this.ctx.player) {
    ...
    _d.y += rand(-0.2, 0.3);
    _d.normalize();
    const sp = spread * (1 + P.speed * 0.06);
    _d.x += rand(-sp, sp);
    _d.y += rand(-sp, sp);
    _d.z += rand(-sp, sp);
    _d.normalize();
  ...
  _animateFlyer(e, dt) {
    const J = e.J,
      b = e.body;
    e.phase += dt * 14;
    const flap = Math.sin(e.phase) * 0.35;
  ...
  _animate(e, dt, pc) {
    ...
    e.walk = damp(e.walk, clamp(sp / 4, 0, 1), 10, dt);
    const w = e.walk;
    e.phase += dt * (sp * 2.2 + (sp > 0.4 ? 3 : 0));
    const s = Math.sin(e.phase),
      c = Math.cos(e.phase);
    J.legL.rotation.x = s * 0.9 * w;
    J.legR.rotation.x = -s * 0.9 * w;
    J.shinL.rotation.x = Math.max(0, c) * 1.1 * w;
    J.shinR.rotation.x = Math.max(0, -c) * 1.1 * w;
    if (!b.onGround) {
      J.legL.rotation.x = -0.5;
      J.legR.rotation.x = 0.6;
      J.shinL.rotation.x = 1.0;
      J.shinR.rotation.x = 0.5;
    }
    if (T.weapon === 'bomb' || T.model === 'blob') {
      J.armL.rotation.x = -2.4 + Math.sin(e.t * 20) * 0.4 * w;
      J.armR.rotation.x = -2.4 - Math.sin(e.t * 20) * 0.4 * w;
      J.torso.rotation.z = Math.sin(e.phase) * 0.12 * w;
      J.torso.rotation.x = -0.15 * w;
      if (J.spark) J.spark.scale.setScalar(0.7 + Math.random() * 0.8 + (e.fuseT >= 0 ? 1.5 : 0));
      J.hips.position.y = 0.5 + Math.abs(c) * 0.08 * w;
      if (e.fuseT >= 0) {
        J.torso.rotation.x = Math.sin(e.t * 40) * 0.15;
        J.headG.rotation.y = Math.sin(e.t * 30) * 0.3;
      }
      return;
    }
    const aim = e.aimAmt;
    if (T.weapon === 'blade') {
      ...
      let raise = 0,
        slam = 0;
      if (a && a.kind === 'stomp') {
        raise = a.t < 0.75 ? clamp(a.t / 0.6, 0, 1) : 0;
        slam = a.t >= 0.75 ? 1 : 0;
      }
      if (a && a.kind === 'throw') {
        raise = a.t < 0.6 ? clamp(a.t / 0.5, 0, 1) : 0;
      }
      J.armR.rotation.x = -0.4 - s * 0.5 * w - raise * 2.4 + slam * 0.9;
      J.armR.rotation.z = -0.35;
      J.foreR.rotation.x = -0.6 - raise * 0.4;
      ...
      if (T.shield) {
        J.armL.rotation.x = -1.2;
        J.armL.rotation.y = 0.3;
        J.foreL.rotation.x = -0.9;
      } else {
        J.armL.rotation.x = s * 1.0 * w * (1 - aim) + -1.2 * aim;
        J.armL.rotation.y = 0.6 * aim;
    ...
    if (e.state === 'stunned') {
      ...
      J.armR.rotation.x = -2.5;
      J.legL.rotation.x = -0.8;
      J.legR.rotation.x = 0.9;
    }
  }
}
