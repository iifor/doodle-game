...
export class World {
  constructor(cell = 8) {
    if (!Number.isFinite(cell) || cell <= 0) throw new RangeError('World cell must be positive');
    this.boxes = [];
    this.cell = cell;
    this.grid = new Map();
    this._qid = 0;
  ...
  _insert(b) {
    const c = this.cell;
    const x0 = Math.floor(b.min.x / c),
      x1 = Math.floor(b.max.x / c),
      z0 = Math.floor(b.min.z / c),
      z1 = Math.floor(b.max.z / c);
  ...
  query(min, max, out = []) {
    ...
    const x0 = Math.floor(min.x / c),
      x1 = Math.floor(max.x / c),
      z0 = Math.floor(min.z / c),
      z1 = Math.floor(max.z / c);
    for (let ix = x0; ix <= x1; ix++)
      for (let iz = z0; iz <= z1; iz++) {
  ...
  overlapsAABB(min, max) {
    return this.query(min, max, _out).length > 0;
  }
  ...
  overlapsBody(body) {
    this._bodyBox(body, _min, _max);
    return this.query(_min, _max, _out).length > 0;
  }
  ...
  _resolveAxis(body, axis, move) {
    ...
    const lim = Math.abs(move) + 0.03;
    for (let iter = 0; iter < 4; iter++) {
      this._bodyBox(body, _min, _max);
      this.query(_min, _max, _out);
  ...
  _moveHoriz(body, dx, dz, canStep) {
    ...
    if (dx === 0 && dz === 0) return;
    p.x += dx;
    const rx = this._resolveAxis(body, 0, dx);
    p.z += dz;
    const rz = this._resolveAxis(body, 2, dz);
    if (rx === 0 && rz === 0) return;
    const nx = p.x,
      nz = p.z;
    let stepped = false;
    if (canStep && body.stepHeight > 0) {
      p.x = ox;
      p.z = oz;
      p.y = oy + body.stepHeight;
      if (!this.overlapsBody(body)) {
        p.x += dx;
        const rx2 = this._resolveAxis(body, 0, dx);
        p.z += dz;
        const rz2 = this._resolveAxis(body, 2, dz);
        p.y -= body.stepHeight;
        this._resolveAxis(body, 1, -body.stepHeight);
        const d1 = (nx - ox) ** 2 + (nz - oz) ** 2,
          d2 = (p.x - ox) ** 2 + (p.z - oz) ** 2;
  ...
  moveBody(body, dt) {
    if (!Number.isFinite(dt) || dt < 0 || dt > 0.05) throw new RangeError(`Invalid physics timestep: ${dt}`);
    if (![...body.pos, ...body.vel].every(Number.isFinite))
      throw new Error('Non-finite body position or velocity');
    const sp = body.vel.length();
    const maxStep = Math.max(0.2, body.halfW * 0.8);
    const n = Math.max(1, Math.ceil((sp * dt) / maxStep));
    if (!Number.isSafeInteger(n) || n > 512)
      throw new RangeError('Physics movement exceeds supported substep budget');
    if (n === 1) {
      this._moveBodyStep(body, dt);
      return;
    }
    let landVel = 0,
      hitWall = false,
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (body.hitWall) {
        hitWall = true;
        wx = body.wallNormal.x;
        wz = body.wallNormal.z;
      }
      if (body.hitCeiling) hitCeiling = true;
      if (body.landVel) landVel = body.landVel;
    }
    body.hitWall = hitWall;
    body.wallNormal.set(wx, 0, wz);
    body.hitCeiling = hitCeiling;
    body.landVel = landVel;
  }
